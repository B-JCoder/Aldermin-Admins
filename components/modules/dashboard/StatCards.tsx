...
export function StatCards() {
  ...
  return (
    <AdminCard className="p-4 sm:p-5 h-full flex flex-col justify-between gap-3 sm:gap-4">
      {stats.map((stat) => (
        <div
          key={stat.label}
          className={cn(
        ...
        >
          ...
          <div
            className={cn(
              "absolute -right-4 -bottom-4 w-16 h-16 rounded-full blur-2xl opacity-50",
              stat.color === "yellow" && "bg-yellow-300",
              stat.color === "purple" && "bg-purple-300",
              stat.color === "orange" && "bg-orange-300",
            )}
          ></div>
        </div>
      ))}
